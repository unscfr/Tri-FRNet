...
from ultralytics.nn.modules.new import MODEL
...
class SimplifiedMBWTConv2d(nn.Module):
    def __init__(self, in_channels, out_channels,
                 wt_type='db1', ssm_ratio=1, forward_type="v05"):
        super().__init__()
        ...
        self.wt_transform = partial(wavelet_transform, filters=self.wt_filter)
        self.iwt_transform = partial(inverse_wavelet_transform, filters=self.iwt_filter)
    ...
    def forward(self, x):
        # 小波分解
        x_wt = self.wt_transform(x)  # [B, C, 4, H/2, W/2]

        # 仅保留低频分量
        x_ll = x_wt[:, :, 0, :, :]  # [B, C, H/2, W/2]

        # SS2D处理
        x_ss2d = self.ss2d(x_ll) * self.scale

        # 小波重构（仅用处理后的低频分量）
        reconstructed = self.iwt_transform(
            torch.cat([x_ss2d.unsqueeze(2), x_wt[:, :, 1:, :, :]], dim=2)
        )
