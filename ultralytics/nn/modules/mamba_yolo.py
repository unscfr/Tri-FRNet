...
class FR(nn.Module):
    def __init__(
        ...
        # ==========SSM============================
        self.norm = norm_layer(hidden_dim)
        self.ss2d = SS2D(d_model=self.hidden_dim,
                         d_state=ssm_d_state,
                         ssm_ratio=ssm_ratio,
                         ssm_rank_ratio=ssm_rank_ratio,
                         dt_rank=ssm_dt_rank,
        ...
        self.act = nn.SiLU()
        self.cv1 = nn.Conv2d(hidden_dim, hidden_dim // 2, kernel_size=1)
        self.cv2 = nn.Conv2d(hidden_dim, hidden_dim // 2, kernel_size=1)
        self.cv3 = nn.Conv2d(hidden_dim, hidden_dim, kernel_size=1)
...
class FRM(nn.Module):
    def __init__(
        ...
        if self.mlp_branch:
            ...
            self.mlp = RGBlock(in_features=hidden_dim, hidden_features=mlp_hidden_dim, act_layer=mlp_act_layer,
                               drop=mlp_drop_rate)
        self.cv1 = Conv(hidden_dim, hidden_dim, 1)
        self.act = nn.SiLU()
        self.eca = ECA(hidden_dim)
    ...
    def forward(self, x):
        ...
        # print(X1.shape)
        # input = input + self.drop_path(self.ss2d(self.norm(X1)))
        x1 = x + self.drop_path(self.ss2d(self.norm(x)))
        x1 = self.norm(self.cv1(x1))
        x1 = self.act(x1)
        # # ===================
        x2 = self.eca(x)
        x2 = self.cv1(x2) + x
...
class XBlock(nn.Module):
    ...
    def forward(self, input):
        ...
        X1 = self.lsblock(input)
        #print(X1.shape)
        input = input + self.drop_path(self.norm(X1))
        # ===================
        if self.mlp_branch:
            #print(input.shape)
            input = input + self.drop_path(self.mlp(self.norm2(input)))
